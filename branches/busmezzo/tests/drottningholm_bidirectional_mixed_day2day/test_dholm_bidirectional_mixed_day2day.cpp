    ...
    */
    ...
    for(auto od : ods)
    {
        ...
        for(auto path : pathset)
        {
            ...
            QVERIFY(n_transfers < 2); // there should be no more than 1 transfer for any path for choice set params defined
            
            //!< check that transit links and pick-up and drop-off stops match case description
            Busline* first_leg = alt_lines.front().front();           
            Busstop* first_pickup_stop = first_leg->stops.front();
            Busstop* first_dropoff_stop = first_leg->stops.back();
            int o1 = first_pickup_stop->get_id();
            int d1 = first_dropoff_stop->get_id();
            
            Busline* second_leg = nullptr;
            Busstop* second_pickup_stop = nullptr;
            ...
            if(n_transfers > 0)
            {
                ...
                second_dropoff_stop = second_leg->stops.back();
                o2 = second_pickup_stop->get_id();
                d2 = second_dropoff_stop->get_id();
            }
            ...
            if(od_category == ODCategory::b2b) //branch2branch
            {
                 ...
                 * DRT -> direct
                 * FIX -> direct
                 * FIX -> transfer -> DRT (x should be excluded based on path set generation params)
                */
                if(first_leg->is_flex_line())
                {
                    QVERIFY(is_on_branch(o1));
                    QVERIFY(is_on_branch(d1));
                    QVERIFY(second_leg == nullptr);
                }
                else // leg is fixed
                {
                    QVERIFY(is_on_branch(o1)); 
...
//                    {
//                        QVERIFY(is_on_branch(d1));
//                        QVERIFY(is_on_branch(o2));
//                        QVERIFY(second_leg->is_flex_line());
//                        QVERIFY(is_on_branch(d2));
//                    }
                    QVERIFY(is_on_branch(d1));
                    QVERIFY(second_leg == nullptr);
            ...
            if(od_category == ODCategory::b2c) //branch2corridor
            {
                 ...
                 * DRT -> transfer -> FIX
                 * FIX -> direct
                 * FIX -> transfer -> FIX (x should be excluded based on path set generation params)
                */
                if(first_leg->is_flex_line())
                {
                    QVERIFY(is_on_branch(o1));
                    QVERIFY(is_transfer_stop(d1));
                    QVERIFY(second_leg != nullptr);
                    QVERIFY(!second_leg->is_flex_line());
                    QVERIFY(is_transfer_stop(o2));
                    QVERIFY(is_on_corridor(d2));
                }
                else // leg is fixed
                {
                    QVERIFY(is_on_branch(o1)); 
...
//                    }
            ...
            if(od_category == ODCategory::c2c) //corridor2corridor
            {
                 ...
                 * FIX -> direct
                */
                QVERIFY(!first_leg->is_flex_line());
                QVERIFY(is_on_corridor(o1));
                QVERIFY(is_on_corridor(d1)); 
                QVERIFY(second_leg == nullptr);
            }
            if(od_category == ODCategory::c2b) //corridor2branch
            {
                /*
                 * FIX -> direct
                 * FIX -> transfer stop anywhere on branches -> DRT (x should be excluded based on path set generation params)
                 * FIX -> transfer -> FIX (x should be excluded based on path set generation params)
                */
                QVERIFY(!first_leg->is_flex_line());             
                QVERIFY(is_on_corridor(o1)); 
                if(n_transfers > 0)
                {
                    qDebug() << "debugging corridor 2 branch transfers";
                    qDebug() << "second leg (pick-up,drop-off): ("<<o2<<","<<d2<<")";
                }
            }
