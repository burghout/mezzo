...
//!< Pentagon feeder network with only fixed service available
const std::string network_path = "../networks/PentaFeeder_fixed/";

const std::string network_name = "masterfile.mezzo";

const QString expected_outputs_path = ":/networks/PentaFeeder_fixed/ExpectedOutputs/";
...
class TestPentaFeeder_fixed : public QObject
{
    Q_OBJECT

public:
    TestPentaFeeder_fixed(){}
...
void TestPentaFeeder_fixed::testCreateNetwork()
{
...
void TestPentaFeeder_fixed::testInitNetwork()
{
    ...
    QVERIFY2 (theParameters->drt == false, "Failure, DRT is set to true in parameters for PentaFeeder_fixed");
    QVERIFY2 (AproxEqual(net->get_currenttime(),0.0), "Failure, currenttime should be 0 at start of simulation");

    QVERIFY(theParameters->choice_set_indicator == 1); //paths already defined

    map<int,Controlcenter*> ccmap = net->get_controlcenters();
    QVERIFY2(ccmap.size() == 0, "Failure, network should have 0 controlcenters");

    QVERIFY2(net->get_buslines().size() == 2, "Failure, network should have 2 buslines defined");
    QVERIFY2(AproxEqual(net->get_buslines()[0]->get_planned_headway(),720.0), "Failure, buslines should have a 720 second (12 min) planned headway");

    QVERIFY2(net->get_busvehicles().size() == 30, "Failure, network should have 30 scheduled vehicles"); // even when chained together with the same vehicle ID a vehicle is created for each scheduled trip
    QVERIFY2(net->get_busvehicles()[0]->get_capacity()==50,"Failure, buses should have capacity 50");
...
void TestPentaFeeder_fixed::testRunNetwork()
{
...
void TestPentaFeeder_fixed::testSaveResults()
{
...
void TestPentaFeeder_fixed::testDelete()
{
    delete nt;
...
QTEST_APPLESS_MAIN(TestPentaFeeder_fixed)

#include "test_pentafeeder_fixed.moc"
