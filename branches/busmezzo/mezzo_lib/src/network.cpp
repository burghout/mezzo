            ...
            if (o_stop->get_id() != d_stop->get_id() && o_stop->check_destination_stop(d_stop))
            {
                ...
                if (!path_set.empty())
                {
                    ...
                    for (auto path : path_set)
                    {
                        ...
                        ODCategory od_category = get_od_category(p_o_stop->get_id(), p_d_stop->get_id());
                        PathType path_type = Pass_path::get_pathtype(path); // @note 'other' path type category corresponds to paths with less than 1 leg, more than 2 legs, or any mixed mode legs
                        if(path_type == PathType::other) // always delete the 'other' category
                        {
                            paths_to_be_deleted[path] = true;
                        ...
                        case ODCategory::b2b:
                            switch (path_type)
                            {
                            case PathType::fix: break; // only direct paths available
                            case PathType::drt: break;
                            case PathType::drt_drt:
                            case PathType::fix_drt:
                            case PathType::drt_fix:
                            case PathType::fix_fix:
                                paths_to_be_deleted[path] = true;
                                ++filtered_path_counts[od_category][path_type];
                                break;
                            default: ;
                        ...
                        case ODCategory::b2c:
                            switch (path_type)
                            {
                            case PathType::fix: break; // FIX-> and DRT->FIX available
                            case PathType::drt_fix:
...
//    int total_filter_categories = b2c_drt_to_first_common_stop_only+c2b_drt_from_first_common_stop_only;
    ...
    assert(filtered_path_count == total_filter_categories);
        ...
        if (fwf_wip::day2day_drt_no_rti)
        {
            ...
            if (day == 1)
            {
                ...
                if(PARTC::drottningholm_case)
                {
                    ...
                    day2day->write_convergence_per_od_and_mode_header(workingdir + "o_fwf_convergence_odcategory_mode.dat");
            ...
            if(PARTC::drottningholm_case)
            {
                ...
                day2day->write_convergence_per_od_and_mode(workingdir + "o_fwf_convergence_odcategory_mode.dat");
