...
//	double calc_next_scheduled_arrival_at_stop (Busstop* stop, double time);		//!< returns the remaining time till the next trip on this line is scheduled to arrive at a stop (according to schedule only) 
    ...
    */
	...
	void add_stop_delta(Busstop* stop, double delta_from_preceding_stop) { delta_at_stops.push_back(make_pair(stop, delta_from_preceding_stop)); } //!< add a scheduled/expected travel time between stops on this line (starting from 0 for the first stop)
...
//	int vtype;					//!< vehicle type. There are usually multiple types of Busses
    ...
    */
    ...
    double planned_headway; //!< planned headway in seconds of this line, set to zero by default in constructor. If this is a DRT line then the planned headway is not based on pre-scheduled trips but on expectations given service design (@todo DRT currently only valid with trip format 3 now!)
    /**@}*/
