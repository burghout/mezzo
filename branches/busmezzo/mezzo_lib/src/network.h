...
public:
  ...
  */
    ...
    vector<Busstop*> get_busstops_on_link(Link* link) const; //!< returns a vector of any busstops located on the link
    vector<Busstop*> get_busstops_on_busroute(Busroute* route) const; //!< returns a vector of any busstops located on the route in order of when they were encountered from the first link to the last link
    vector<Busstop*> get_intermediate_busstops_on_busroute(Busroute* route, Busstop* start_stop, Busstop* end_stop) const; //!< returns a vector of any busstops located on route between start- and end-stop in the order of when they are encountered. Includes start- and end-stop in returned vector.
    /**}*/
    ...
    // Wilco 2020: add mapping to and from Graph nodes and links.
    int get_node_to_graphnode(const int n) {return node_to_graphnode[n];}
    int get_graphnode_to_node(const int gn) {return graphnode_to_node[gn];}
    int get_link_to_graphlink(const int l) {return link_to_graphlink[l];}
    int get_graphlink_to_link(const int gl) {return graphlink_to_link[gl];}
#ifndef _NO_GUI
