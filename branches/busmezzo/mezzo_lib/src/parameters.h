...
extern double drt_exploration_wt; //!< default value returns by Controlcenter if RTI or experience is unavailable for passenger anticipated waiting time calculations for decisions
...
namespace fwf_wip
{
    ...
    extern bool write_all_d2d_alphas; // true if disaggregate learning variables for individual ODSL and ODSLL components should be written to output files
...
namespace PARTC
{
    ...
    extern pair<Busstop*,Busstop*> transfer_stops;
    const pair<int,int> transfer_stop_ids = std::make_pair(277024, 77024);
...
// #moe_parameters
 ...
 // transit demand parameters:
    ...
    int max_days = 20; // day2day termination criteria if break_criterium not met, maximum number of days we iterate over. 
    ...
    double gate_generation_time_diff = 0.0; // passengers are generated slightly before service leaves to account for boarding time
    int empirical_demand = 0; //!< only relevant for demand format 3, default value is 0. Value of 1 indicates that empirical demand should be read from a "transit_demand_empirical.dat" input file. If 0 this file is ignored.
...
// transit control parameters
    ...
    bool transfer_sync = false; //!< true if transfer synchronization is being used. Will effect the way the line definition in transit_network.dat is read.

    bool drt = false; //!< flag for DRT vehicles with associated control center, when true will look for additional input in transit_fleet and transit_network
