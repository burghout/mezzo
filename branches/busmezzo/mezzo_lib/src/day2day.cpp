...
//	static ofstream out("MELINA_OUTPUT.dat", ios_base::app);
//	out.rdbuf()->pubsetbuf(my_buffer, buf_size);
//	static string header = "";
//	if (header == "") {
//		std::cout << "SAVING RESULTS IN OUTPUT FILE 'MELINA_OUTPUT.dat' - PLEASE WAIT..." << std::endl;
//		header = "Day\tPass id\tTrip_id\tLine_id\tOriginal_origin\tDestination_stop\tStop.get_origin\
//tStop_id_(leg)\tCar_id\tExcpected_ivt\tExperienced_ivt\tCrowding";
//		out << header << std::endl;
//	}
//	out << day << "\t"
//		<< pass_id << "\t"
	...
	for (vector<ODstops*>::iterator od_iter = odstops.begin(); od_iter != odstops.end(); od_iter++)
	{
		...
		for (map<Passenger*,list<Pass_onboard_experience> >::iterator pass_iter1 = pass_list.begin(); pass_iter1 != pass_list.end(); pass_iter1++)
		{
			...
			for (list<Pass_onboard_experience>::iterator exp_iter = onboard_experience_list.begin(); exp_iter != onboard_experience_list.end(); exp_iter++)
			{
				...
				//Melina
				//if (exp_iter->trip_id==1410) {
				//	std::cout << "Passenger: " << exp_iter->pass_id << ", Trip: " << exp_iter->trip_id
				//		<< ", Stop: " << exp_iter->stop_id << ", Car: " << exp_iter->car_id << "\n"
				//		<< "Origin: " << exp_iter->original_origin << ", Destination: " << exp_iter->destination_stop
				//		<< "\n" << "Total expected ivtt: " << exp_iter->expected_ivt
				//		<< "\n" << "Total experienced ivtt: " << exp_iter->experienced_ivt.first
				//		<< "\n" << "Crowding: " << exp_iter->experienced_ivt.second
				//		<< "\n" << endl;
				//}

				////MELINA_START
				//static std::string input = "-888";
				//static int x = -888;
				//bool tryParse = false;
				//if (x == -888)
				//{
				//	std::cout << "***** INPUT *****" << std::endl;
				//	std::cout << "Enter passenger number to print output only for that passenger\n"
				//		<< "or '-1' to print output for all passengers"
				//		<< "\nor '-2' to save output in file 'MELINA_OUTPUT.dat': ";
				...
				//if (x == -1)
				//{
				//	std::cout << "Pass id: " << exp_iter->pass_id
				//		<< "\n-> Trip id: " << exp_iter->trip_id
				//		<< "\n-> Line id: " << exp_iter->line_id
				//		<< "\n-> Original origin: " << exp_iter->original_origin
				//		<< "\n-> Destination stop: " << exp_iter->destination_stop
				//		<< "\n-> Stop.get_origin: " << exp_iter->stop_id
				//		<< "\n-> Stop id (leg): " << exp_iter->leg_id
				//		<< "\n-> Car id: " << exp_iter->car_id
				//		<< "\n--> Excpected ivt: " << exp_iter->expected_ivt
				//		<< "\n--> Experienced ivt: " << exp_iter->experienced_ivt.first
				//		<< "\n--> Crowding: " << exp_iter->experienced_ivt.second
				//		<< "\n" << std::endl;
				//}
				//else if (exp_iter->pass_id == x) {
				//	std::cout << "Pass id: " << exp_iter->pass_id
				//		<< "\n-> Trip id: " << exp_iter->trip_id
				//		<< "\n-> Line id: " << exp_iter->line_id
				//		<< "\n-> Original origin: " << exp_iter->original_origin
				...
				//	if (curr_day != day) {
				//		curr_day = day;
				//		std::cout << "\tSAVING DAY " << curr_day << "..." << std::endl;
				//	}
				//	melina_save(
				//		day,
				//		exp_iter->pass_id,
				//		exp_iter->trip_id,
				//		exp_iter->line_id,
				//		exp_iter->original_origin,
				//		exp_iter->destination_stop,
				//		exp_iter->stop_id,
				//		exp_iter->leg_id,
				//		exp_iter->car_id,
				//		exp_iter->expected_ivt,
				//		exp_iter->experienced_ivt.first,
				//		exp_iter->experienced_ivt.second
				//	);
				//}
				////MELINA_END
